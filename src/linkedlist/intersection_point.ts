...
function findIntersectionPoint(l1:Node, l2:Node):Node{
    var length1 = findLength(l1), length2 = findLength(l2)
    var diff = Math.abs(length1 - length2)
    var temp1 = l1, temp2 = l2;
    if(length1>length2) temp1 = moveNTimes(l1, diff)
    else temp2 = moveNTimes(l2, diff)
...
function findLength(head:Node):number{
    if(head == null) return 0;
    return 1+findLength(head.next)
}

function moveNTimes(head:Node, n:number):Node{
